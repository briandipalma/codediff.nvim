...
void test_simple_change() {
  ...
  const char *original[] = {"hello world", "unchanged line", "goodbye"};

  const char *modified[] = {"hello universe", "unchanged line", "goodbye"};

  DiffOptions opts = {.ignore_trim_whitespace = false,
                      .max_computation_time_ms = 0,
                      .compute_moves = false,
                      .extend_to_subwords = true};

  RenderPlan *plan = compute_diff_render_plan(original, 3, modified, 3, &opts);
...
void test_empty() {
  ...
  RenderPlan *plan = compute_diff_render_plan(original, 1, modified, 1, &opts);
