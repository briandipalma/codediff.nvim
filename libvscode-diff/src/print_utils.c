...
void diff_core_print_render_plan(const RenderPlan *plan) {
  ...
  for (int i = 0; i < plan->left.line_count; i++) {
    ...
    if (meta->type == HL_LINE_INSERT)
      type_str = "INSERT";
    else if (meta->type == HL_LINE_DELETE)
      type_str = "DELETE";
    ...
    for (int j = 0; j < meta->char_highlight_count; j++) {
      ...
      if (ch->type == HL_CHAR_INSERT)
        ch_type_str = "INSERT";
      else if (ch->type == HL_CHAR_DELETE)
        ch_type_str = "DELETE";

      printf("    [%d] Line %d, cols %d-%d: %s\n", j, ch->line_num, ch->start_col, ch->end_col,
             ch_type_str);
    }
  }
